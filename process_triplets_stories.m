...
sub_preposition = zeros(153809,1);
obj_preposition = zeros(153809,1);
...
emptycells_sub = cellfun(@isempty,sub);
emptycells_verb = cellfun(@isempty,verb);
emptycells_obj = cellfun(@isempty,obj);
index_empty_s = find(emptycells_sub==1);
index_empty_v = find(emptycells_verb==1);
index_empty_o = find(emptycells_obj==1);
index_emptycells = unique([index_empty_s ;index_empty_v;index_empty_o]);
triplets(index_emptycells,:)=[];
sub = triplets(:,1);
verb = triplets(:,2);
obj  = triplets(:,3);
...
junk_sub={'''s';'0';'0rd';'a';'ab';'ablaj';'abu';'adl';'afp';'al';
    ...
    'sa';'sha';'shu';'so';'t';'u';'u0';'u0t';'ud';'v';'ye';'`';'``';'a''far';'a.';'a.d.';'a.h.';'''d';'0\';'be';'0nd'};
index_junk_subs = find(ismember(triplets(:,1),junk_sub));

junk_verb = {'%';'''s';'b';'d';'da';'l';'o';'ye';'~';'ab';'ar';'r'};
index_junk_verbs = find(ismember(triplets(:,2),junk_verb));

junk_obj = {'''s';'-';'0';'>';'a';'ab';'al';'an';'ax';'b';'ba';'do';'h';'km';'o';'qa';'so';'th';'tn';'u';'u0';'ye';'0nd intelligence';'0october';'??!!';'`';'da';'dc';'e';'u0a';'u0ah';'u0bah';'u0cglorified';'u0cterrorists';'u0cthis';'u0he';'u0proof';'u0t';'ubaydis';'ubna0';'ud'};
...
junk_subs = unique_s(frequent_subs(frequent_subs(:,2)<2,1));
junk_subs=[junk_subs ;unique_s(frequent_subs(1:3,1))];
index_junk_subs = find(ismember(triplets(:,1),junk_subs));

junk_verbs = unique_v(frequent_verbs(frequent_verbs(:,2)<2,1));
junk_verbs=[junk_verbs ;unique_v(frequent_verbs(1:3,1))];
index_junk_verbs = find(ismember(triplets(:,2),junk_verbs));

junk_objs = unique_o(frequent_objs(frequent_objs(:,2)<2,1));
junk_objs=[junk_objs ;unique_o(frequent_objs(1:3,1))];
