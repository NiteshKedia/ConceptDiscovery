...
for i=1:length(tripcomp_5_sub)
    ...
    tripcom5{i,1} = sub_words;
    tripcom5{i,2} = verb_words;
    tripcom5{i,3} = obj_words;
    
end

% Serialise the GTs
fifthlevelGT = cell(length(tripcom5),3);
for i = 1 : length(tripcom5)
    for j = 1 : 3
        if (length(tripcom5{i,j}) > 1)
            str = strjoin(tripcom5{i,j}',';');
            fifthlevelGT{i,j} = str;
        else
            fifthlevelGT{i,j} = tripcom5{i,j};
        end
    end
end
display('Fifth Finished');
display('**********************************');