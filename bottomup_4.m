...
for i=1:length(tripcomp_4_sub)
    ...
    tripcom4_test{i,1} = sub_words;
    tripcom4_test{i,2} = verb_words;
    tripcom4_test{i,3} = obj_words;
    
end

% Serialise the GTs
fourthlevelGT_test = cell(length(tripcom4_test),3);
for i = 1 : length(tripcom4_test)
    for j = 1 : 3
        if (length(tripcom4_test{i,j}) > 1)
            str = strjoin(tripcom4_test{i,j}',';');
            fourthlevelGT_test{i,j} = str;
        else
            fourthlevelGT_test{i,j} = tripcom4_test{i,j};
        end
    end
end
