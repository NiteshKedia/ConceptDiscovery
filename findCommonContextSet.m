function [ final_candidate_set ] = findCommonContextSet(sub, verb, obj, tripcomp_sub, tripcomp_verb,tripcomp_obj ,unique_sub,unique_verb,unique_obj)
...
obj_set= unique(x);
%     current_subs = unique_sub(sub);
% current_verb = unique_verb(verb);
% current_objs = unique_obj(obj);
...
if(~isempty(same_subject_set))
    for k=1:length(same_subject_set)
        ...
        if(length(sub)~=length(y))
            ...
            %                 [x,y,z]=find(tripcomp_verb(same_subject_set(k),:));
            %                 verb_words= unique_verb(y);
            %                 [x,y,z]=find(tripcomp_obj(same_subject_set(k),:));
